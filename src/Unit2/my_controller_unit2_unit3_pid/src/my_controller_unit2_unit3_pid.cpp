...
public:
  bool init(hardware_interface::EffortJointInterface *hw, ros::NodeHandle &n) {
    ...
    XmlRpc::XmlRpcValue set_point;
    n.getParam("set_points", set_point);
    spth1_ = double(set_point[0]) * pi_ / 180;
    spth2_ = double(set_point[1]) * pi_ / 180;
  ...
  void update(const ros::Time &time, const ros::Duration &period) {
    ...
    if (firstflag_) {
      ...
      sum1_ += dt * (spth1_ - th1);
      sum2_ += dt * (spth2_ - th2);
      input[0][0] = KP1_ * (spth1_ - th1) + KI1_ * sum1_ + KD1_ * (-dth1);
      input[1][0] = KP2_ * (spth2_ - th2) + KI2_ * sum2_ + KD2_ * (-dth2);
