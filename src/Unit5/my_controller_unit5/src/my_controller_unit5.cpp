...
public:
  ...
  void update(const ros::Time &time, const ros::Duration &period) {
    ...
    if (firstflag_) {
      ...
      double d_spq[2][20];
      d_spq[0][0] = 2 * f1 * cos(f1 * t);
      d_spq[1][0] = -1 * f2 * sin(f2 * t);

      double dd_spq[2][20];
      dd_spq[0][0] = -2 * f1 * f1 * sin(f1 * t);
      dd_spq[1][0] = -1 * f2 * f2 * cos(f2 * t);
      ...
      double M[2][20];
      double aux10[2][20];
      double input_aux[2][20];

      sumMatrices(D, JM, M, 2, 2, 2, 2);
      multiplyMatrices(M, v, aux10, 2, 2, 2, 1);
      sumMatrices(aux10, h, input_aux, 2, 1, 2, 1);

      double JacobianT[2][20];
      JacobianT[0][0] = -sin(th1) * l1 - sin(th1 + th2) * l2;
      JacobianT[1][0] = -sin(th1 + th2) * l2;
      JacobianT[0][1] = cos(th1) * l1 + cos(th1 + th2) * l2;
      JacobianT[1][1] = cos(th1 + th2) * l2;

      double F_dist[2][20] = {{-5000}, {-5000}};
      double aux_force[2][20];
      double input[2][20];

      multiplyMatrices(JacobianT, F_dist, aux_force, 2, 2, 2, 1);
      sumMatrices(aux_force, input_aux, input, 2, 1, 2, 1);
